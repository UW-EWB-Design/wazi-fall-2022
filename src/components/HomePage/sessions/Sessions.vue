<template>
    <div class="Sessions">
        <div
            class="max-w-md mx-auto shadow-md border-2 border-gray-100 rounded-sm overflow-hidden md:max-w-2xl"
        >
            <div class="p-4 w-full">
                ...
                <div v-if="totalSessions === 0" class="p-5 text-sm">
                    <p>
                        You have not had any sessions. Speak with a licensed
                        therapist today!
                    </p>
                </div>
                <!-- Existing sessions -->
                <div v-else>
                    <div class="flex flex-row justify-between my-4">
                        <!-- Session Stats -->
                        <div
                            class="flex-col flex-1 rounded-sm content-center h-auto p-3 border border-gray-200 duration-300 hover:bg-gray-100 cursor-pointer"
                        >
                ...
                <!-- Book a session -->
                <button
                    class="relative font-bold rounded-sm px-6 py-2 my-2 w-full border border-blue-800 text-blue-800 duration-300 hover:bg-blue-600 hover:text-white cursor-pointer"
                >
                    <span class="absolute left-4 text-lg leading-tight">+</span
                    >Book a Session
                </button>
            </div>
            <!-- Recent Sessions -->
            <div v-if="totalSessions > 0" class="pb-2 pr-2 w-full">
                ...
                <div class="pt-2">
                    <a
                        class="text-sm text-blue-500 font-bold cursor-pointer hover:bold"
                        href="#"
                        >See all sessions</a
                    >
                </div>
